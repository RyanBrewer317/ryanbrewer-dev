      ...
      return $result.try$(
        $list.try_map(
          ...
          (_use0) => {
            let path = _use0[0];
            let content = _use0[1];
            let _pipe = $simplifile.write(temp + path, content);
            return $result.map_error(
              _pipe,
              (_capture) => {
          ...
          return $result.try$(
              ...
              return $list.try_map(
                ...
                (route) => {
                  if (route instanceof Static && route.path === "/") {
                    ...
                    let path$1 = $1[0];
                    let name = $1[1];
                    let $2 = $simplifile.create_directory_all(temp + path$1);
            ...
            (_) => {
              return $result.try$(
                (() => {
                  let $1 = (() => {
                    let _pipe = $simplifile.verify_is_directory(out_dir$1);
                ...
                (_) => {
                  return $result.try$(
                    (() => {
                      let _pipe = $simplifile.copy_directory(temp, out_dir$1);
                      return $result.map_error(
                        _pipe,
                        (var0) => { return new CannotWriteToOutputDir(var0); },
                      );
                    ...
                    (_) => {
                      return $result.try$(
                        (() => {
                          let _pipe = $simplifile.delete$(temp);
                          return $result.map_error(
                            _pipe,
                            (var0) => { return new CannotCleanupTempDir(var0); },
                          );
                        })(),
