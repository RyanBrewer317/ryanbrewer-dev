...
-spec component(list(lustre@internals@vdom:attribute(SMI))) -> lustre@internals@vdom:element(SMI).
component(Attrs) ->
...
-spec subscribe(binary(), fun((lustre@internals@patch:patch(SMV)) -> nil)) -> lustre@internals@runtime:action(SMV, lustre:server_component()).
...
-spec do_select(
    fun((fun((SNJ) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(SNJ))
) -> lustre@effect:effect(SNJ).
do_select(Sel) ->
...
-spec select(
    fun((fun((SNE) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(SNE))
) -> lustre@effect:effect(SNE).
select(Sel) ->
    do_select(Sel).

-file("/home/runner/work/lustre/lustre/src/lustre/server_component.gleam", 257).
-spec set_selector(
    gleam@erlang@process:selector(lustre@internals@runtime:action(any(), SNP))
) -> lustre@effect:effect(SNP).
set_selector(_) ->
    lustre@effect:from(
        fun(_) ->
            _pipe = <<"
